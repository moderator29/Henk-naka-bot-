...
-- =============================================================================
-- Roles, moderation & admin foundation (mirrors 0011_roles_and_admin.sql)
-- =============================================================================
...
returns boolean language sql stable security definer set search_path = public as $$
  select exists(select 1 from public.user_roles where user_id = _uid and role in ('moderator','admin','superadmin'));
$$;

alter table public.user_roles enable row level security;
drop policy if exists user_roles_self_read on public.user_roles;
drop policy if exists user_roles_admin_write on public.user_roles;
create policy user_roles_self_read on public.user_roles for select using (auth.uid() = user_id or public.is_staff());
...
create policy admin_audit_staff_read on public.admin_audit_log for select using (public.is_staff());

create table if not exists public.reports (
  id uuid primary key default gen_random_uuid(),
  reporter_id uuid references public.users(id) on delete set null,
  target_type text not null check (target_type in ('post','comment','user','message','profile')),
...
create policy reports_read on public.reports for select using (auth.uid() = reporter_id or public.is_staff());
...
create index if not exists creator_apps_status_idx on public.creator_applications (status, created_at desc);
...
create policy creator_apps_read on public.creator_applications for select using (auth.uid() = user_id or public.is_staff());
create policy creator_apps_insert_self on public.creator_applications for insert with check (auth.uid() = user_id);
create policy creator_apps_staff_update on public.creator_applications for update using (public.is_staff()) with check (public.is_staff());
...
create policy posts_public_read on public.posts for select using (
  auth.uid() = creator_id
  or (moderation_status <> 'removed' and (scheduled_for is null or scheduled_for <= now()))
);
...
-- =============================================================================
-- Broadcast/news + legal (mirrors 0014_news_and_legal.sql)
-- =============================================================================
...
create policy announcement_likes_write on public.announcement_likes for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create index if not exists announcement_comments_idx on public.announcement_comments (announcement_id, created_at);
...
create policy announcement_comments_write on public.announcement_comments for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy legal_admin_write on public.legal_documents for all using (public.is_admin()) with check (public.is_admin());
